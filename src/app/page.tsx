  ...
  useEffect(() => {
    ...
    rules.forEach(rule => {
      rule.addEventListener('mouseenter', () => {
        (rule as HTMLElement).style.boxShadow = '0 5px 15px rgba(166, 124, 82, 0.2)';
        (rule as HTMLElement).style.borderLeft = '3px solid var(--secondary-color)';
        (rule as HTMLElement).style.paddingLeft = '10px';
      });
      
      rule.addEventListener('mouseleave', () => {
        (rule as HTMLElement).style.boxShadow = 'none';
        (rule as HTMLElement).style.borderLeft = 'none';
        (rule as HTMLElement).style.paddingLeft = '0';
      });
    });
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (
        ...
        (header as HTMLElement).style.background = 'var(--primary-color)';
      }
    };
